	...
	useEffect(() => {
		const checkExistingPayment = async () => {
			...
			try {
				...
				const { data: existingPayment, error: existingError } = await supabase.from('payments').select('id, status, amount, email_sent').eq('email', email).maybeSingle();
		...
		if (transactionStatus === 'success' && currentPaymentId && email) {
			const sendEmail = async () => {
				try {
					...
					const { data: payment, error: fetchError } = await supabase.from('payments').select('amount').eq('id', currentPaymentId).single();
	...
	return (
		<div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
			<div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
				...
				<div className="lg:col-span-1">
					<PaymentDetailsCard amount={uniqueAmount || 0} email={email || ''} />
				</div>

				<div className="lg:col-span-1">
					<PaymentInstructionsCard amount={uniqueAmount || 0} merchantAddress={MERCHANT_ADDRESS} transactionStatus={displayStatus} />
				</div>
			</div>
		</div>
	);
